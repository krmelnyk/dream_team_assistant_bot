...
def test_create_contact_with_valid_phone():
    ...
    assert contact.name.value == "Anton"
    assert len(contact.phones) == 1
    assert contact.phones[0].value == "+380991112233"
...
def test_add_contact_to_book():
    ...
    assert book.data["Anton"].phones[0].value == "+380991112233"
...
def test_upcoming_birthdays_returns_contact():
    ...
    contact.set_birthday(birthday_in_past)
    book.add_contact(contact)

    result = book.upcoming_birthdays(7)

    assert len(result) == 1
    assert result[0].name.value == "Anton"


def test_upcoming_birthdays_empty_raises_not_found():
    book = ContactBook()
    contact = Contact("Anton")
    contact.set_phone("+380991112233")
